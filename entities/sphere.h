...
public:
    ...
    Sphere(glm::vec3 pos, float radius, int segments, float opacity, glm::vec3 color) {
        position = pos;
        ...
        for (int lat = 0; lat <= latitudeSegments; ++lat) {
            ...
            for (int lon = 0; lon <= longitudeSegments; ++lon) {
                ...
                // Create a gradient effect based on the position of the vertex
                float red = color.x;   // Normalize x to [0, 1] range
                float green = color.y; // Normalize y to [0, 1] range
                float blue = color.z;     // Make sure the colors balance for a smooth gradient
